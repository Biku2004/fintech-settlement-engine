...
public final class Batch1SelfTest {
    ...
    private void run() throws Exception {
        ...
        test("forged policy command rejected", this::forgedPolicyCommandRejected);
        test("wrong source event type rejected", this::wrongSourceEventTypeRejected);
        test("oversized forged reversal rejected", this::oversizedForgedReversalRejected);
        test("mixed platform owners rejected", this::mixedPlatformOwnersRejected);
    ...
    private void forgedPolicyCommandRejected() {
        LedgerKernel kernel = kernel();
        PostingCommand forged = PostingCommand.standard(
                context(2_001, "forged-policy", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED),
                CapturePostingPolicy.POLICY,
                List.of(
                        new PostingLine(CASH, EntryDirection.DEBIT, money(1_000)),
                        new PostingLine(PAYABLE, EntryDirection.CREDIT, money(1_000))));
        expect(InvalidPostingPolicyException.class, () -> kernel.post(forged));
        check(kernel.transactionCount() == 0, "forged policy creates no transaction");
    }

    private void wrongSourceEventTypeRejected() {
        expect(InvalidPostingPolicyException.class, () -> new CapturePostingPolicy().create(
                context(2_002, "wrong-source", FinancialEventType.SETTLEMENT_CONFIRMED),
                PROCESSOR, PAYABLE, FEE, money(1_000), money(100)));

        LedgerKernel kernel = kernel();
        PostingCommand direct = PostingCommand.standard(
                context(2_004, "wrong-source-direct", FinancialEventType.SETTLEMENT_CONFIRMED),
                CapturePostingPolicy.POLICY,
                List.of(
                        new PostingLine(PROCESSOR, EntryDirection.DEBIT, money(1_000)),
                        new PostingLine(PAYABLE, EntryDirection.CREDIT, money(900)),
                        new PostingLine(FEE, EntryDirection.CREDIT, money(100))));
        expect(InvalidPostingPolicyException.class, () -> kernel.post(direct));
        check(kernel.transactionCount() == 0, "wrong source direct command creates no transaction");
    }

    private void oversizedForgedReversalRejected() {
        LedgerKernel kernel = kernel();
        LedgerTransaction original = kernel.post(capture(2_005, "oversized-original", 1_000, 100)).transaction();
        PostingCommand forged = PostingCommand.reversal(
                context(2_006, "oversized-reversal", FinancialEventType.REVERSAL_CONFIRMED),
                ReversalPostingPolicy.POLICY,
                List.of(
                        new PostingLine(PROCESSOR, EntryDirection.CREDIT, money(200)),
                        new PostingLine(PROCESSOR, EntryDirection.CREDIT, money(200)),
                        new PostingLine(PAYABLE, EntryDirection.DEBIT, money(200)),
                        new PostingLine(PAYABLE, EntryDirection.DEBIT, money(200)),
                        new PostingLine(FEE, EntryDirection.DEBIT, money(200))),
                original.id());
        expect(InvalidPostingPolicyException.class, () -> kernel.post(forged));
        check(kernel.transactionCount() == 1, "oversized reversal creates no transaction");
    }

    private void mixedPlatformOwnersRejected() {
        LedgerKernel kernel = kernel();
        LedgerAccount foreignFee = account(
                2_003,
                LedgerAccountRole.PLATFORM_FEE_REVENUE,
                AccountScope.platform(uuid(9_999)),
                USD);
        expect(InvalidLedgerAccountException.class, () -> kernel.registerAccount(foreignFee));
        check(kernel.transactionCount() == 0, "foreign platform account is not registered");
    }

    private void unbalancedRejected() {
        LedgerKernel kernel = kernel();
        PostingCommand command = PostingCommand.standard(
                context(3, "bad-balance", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED),
                CapturePostingPolicy.POLICY,
                List.of(
        ...
        check(kernel.transactionCount() == 0, "unbalanced command creates no transaction");
    }

    private void oneSidedRejected() {
        LedgerKernel kernel = kernel();
        PostingCommand command = PostingCommand.standard(
                context(4, "one-sided", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED),
                CapturePostingPolicy.POLICY,
                List.of(
                        new PostingLine(PROCESSOR, EntryDirection.DEBIT, money(50)),
                        new PostingLine(CASH, EntryDirection.DEBIT, money(50))));
        expect(InvalidPostingPolicyException.class, () -> kernel.post(command));
        check(kernel.transactionCount() == 0, "one-sided command creates no transaction");
    }

    private void deterministicEntryOrder() {
        LedgerKernel firstKernel = kernel();
        LedgerKernel secondKernel = kernel();
        PostingContext c1 = context(5, "order-a", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED);
        PostingContext c2 = context(6, "order-b", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED);
        var linesA = List.of(
        ...
        var a = firstKernel.post(PostingCommand.standard(c1, CapturePostingPolicy.POLICY, linesA)).transaction();
        var b = secondKernel.post(PostingCommand.standard(c2, CapturePostingPolicy.POLICY, linesB)).transaction();
    ...
    private void unknownAccountRejected() {
        ...
        PostingCommand command = new CapturePostingPolicy().create(
                context(28, "unknown", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED),
                unknown, PAYABLE, FEE, money(100), money(0));
        expect(UnknownLedgerAccountException.class, () -> kernel.post(command));
    }

    private void closedAccountRejected() {
        LedgerKernel kernel = kernel();
        LedgerAccount closed = new LedgerAccount(
                new LedgerAccountId(uuid(888)), PROCESSOR.role(), PROCESSOR.scope(), USD, LedgerAccountStatus.CLOSED);
        kernel.registerAccount(closed);
        PostingCommand command = new CapturePostingPolicy().create(
                context(29, "closed", FinancialEventType.PAYMENT_CAPTURE_CONFIRMED),
                closed, PAYABLE, FEE, money(100), money(0));
    ...
    private LedgerKernel kernel() {
        LedgerKernel kernel = new LedgerKernel(new PlatformId(PLATFORM_ID), Clock.fixed(TIME, ZoneOffset.UTC));
